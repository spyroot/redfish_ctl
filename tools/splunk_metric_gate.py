...
def query_metric(
    ...
    :return: dict with total ``count``, ``active`` series count,
        ``active_complete`` payload validity, compatibility ``newest_ms``,
        and raw ``results`` for build-identity validation.
...
def run_gate(argv: list[str] | None = None) -> int:
    ...
    fleet_failures = 0
    if expected_hosts:
        fleet_failures = check_build_revision_fleet(
            realm=realm,
            token=token,
    ...
    all_metrics_flowing = counts["FLOWING"] == len(metrics)
    return 0 if all_metrics_flowing and not fleet_failures else 1
