...
# Refuses unless the pipeline is protected (CI_COMMIT_REF_PROTECTED=true, set by GitLab itself), and
# always refuses a merge-request pipeline. There is no opt-out variable: an override the pipeline can set
# would make this gate advisory. This is the last line stopping an apply from an MR pipeline.
set -euo pipefail
