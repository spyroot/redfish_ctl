#!/usr/bin/env bash
# Gate gitlab.project-token.exists: validate the GitLab CI project access token (shared, integration
...
set -euo pipefail
cd "$(dirname "$0")/../../.."
exec python3 tools/gitlab_project_token_gate.py --check exists
