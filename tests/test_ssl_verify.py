...
def test_internal_verify_flag_is_inverse_of_insecure():
    ...
    assert CommandBase(insecure=True)._is_verify_cert is False
    assert CommandBase(insecure=False)._is_verify_cert is True
    # Default (no flag) must skip verification.
    assert RedfishManager()._is_verify_cert is False
    assert CommandBase()._is_verify_cert is False
...
def test_cli_verify_ssl_flag_drives_insecure():
    ...
    CommandBase from the parsed ``verify_ssl`` flag, without constructing a real
