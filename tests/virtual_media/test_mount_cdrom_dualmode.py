...
def test_mount_cdrom_auto_selects_cd_device_on_gb300(
    ...
    result = manager.sync_invoke(
        *_INSERT, uri_path="http://example.test/gb300.iso", confirm=True
    )
...
def test_mount_cdrom_dry_run_by_default(redfish_mock_factory):
    """Without --confirm the command is a dry run: it reports the plan, no POST."""
    manager, service = redfish_mock_factory("supermicro")
    ...
    assert result.data["device"] == "USB1"
    assert result.data["image"] == "http://example.test/gb300.iso"
    assert all(request.method == "GET" for request in service.requests)
