...
``base_post``/``base_patch`` return ``(CommandResult, RedfishCommandRespond)``.
``RedfishCommandRespond`` is an Enum, so ``api_resp.Success`` is attribute access
...
from redfish_ctl.command_shared import RedfishCommandRespond
...
def _error_write(*_args, **_kwargs):
    ...
    return (
        ...
        RedfishCommandRespond.Error,
    )


def _ok_write(*_args, **_kwargs):
    """A plain successful write with no task attached (HTTP 200/204)."""
    return CommandResult({}, None, None, None), RedfishCommandRespond.Ok
...
def test_bios_change_failed_patch_does_not_commit(tmp_path, monkeypatch):
    ...
    This was live: RedfishCommandRespond.Error satisfied ``api_resp.Success or
...
def test_system_import_task_reads_task_id_key(tmp_path, monkeypatch):
    ...
    def accepted_post(*_args, **_kwargs):
        return (
            ...
            RedfishCommandRespond.AcceptedTaskGenerated,
        )
