...
def test_manager_reboot_wait_attaches_wait_block(redfish_mock_factory, monkeypatch):
    ...
    mgr, _ = redfish_mock_factory("hpe")
    monkeypatch.setattr(CommandBase, "idrac_members", "/redfish/v1/Managers/1", raising=False)
    monkeypatch.setattr(
        CommandBase, "base_post",
        lambda self, *a, **k: (CommandResult({"Status": "ok"}, None, None, None), RedfishCommandRespond.Ok))
    monkeypatch.setattr(
        cw, "wait_reachable",
