...
- `CommandBase`, defined in `redfish_ctl/base_manager.py`, for Dell/iDRAC behavior and host-system
  selection.
...
now in `CommandBase`: on a GB300, host actions go to `System_0`, not the HGX baseboard member. On
