...
| `dell-license-queries` | List Dell OEM license-management actions from Manager `DellLicenseManagementService` or run the read-only `ShowLicenseBits` query when selected. | Read |
