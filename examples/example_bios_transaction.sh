...
redfish_ctl bios-change --from_spec change.json on-reset -r

# 3) If the box misbehaves, roll back to the captured restore point
redfish_ctl bios-change --from_spec rollback.json on-reset -r

# Variants:
#   redfish_ctl bios-snapshot --attr_name ProcCStates,LogicalProc -f rollback.json  # by name
#   redfish_ctl bios-snapshot -f full_restore.json                                  # every attribute
