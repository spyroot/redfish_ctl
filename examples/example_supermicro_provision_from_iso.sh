...
#
# Set REDFISH_IP / REDFISH_USERNAME / REDFISH_PASSWORD to the BMC first (see examples/README.md).
...
redfish_ctl vm-mount --host "$SMB_HOST" --path "$ISO_PATH" \
...
redfish_ctl boot-one-shot --device UefiUsbCd

# 4) Reset into the installer.
redfish_ctl system-reset --reset_type ForceRestart --confirm
